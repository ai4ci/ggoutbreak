...
growthrates::england_covid_pcr_positivity %>%
  proportion_locfit_model() %>%
  inner_join(england_ons_infection_survey %>% filter(geography=="England") %>% rename_with(.cols = starts_with("proportion"), .fn = ~stringr::str_replace(.x,"proportion","ons")), by=c("time")) %>%
  transmute(date=date, pre_test_odds = proportion.0.5 / ons.0.5) %>%
  ggplot(aes(x=date,y=pre_test_odds)) + geom_line()
...
```{r}
england_pop = sum(growthrates::england_demographics$population)

growthrates::england_covid_pcr_positivity %>% 
  select(-denom) %>% 
  mutate(denom = england_pop) %>%
  proportion_locfit_model() %>%
  inner_join(england_ons_infection_survey %>% filter(geography=="England") %>% rename_with(.cols = starts_with("proportion"), .fn = ~stringr::str_replace(.x,"proportion","ons")), by=c("time")) %>%
  transmute(date=date, ascertainment = proportion.0.5 / ons.0.5) %>%
  ggplot(aes(x=date,y=ascertainment)) + geom_line()
```
