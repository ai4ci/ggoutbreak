...
If we using a framework for estimating $R_t$ that does not have this requirement
...
tmp = bind_rows(original_disc,cdf_disc) %>% 
...
ggplot2::ggplot(tmp %>% rename(source = disc_type))+
...
R_t_summ = R_t %>% group_by(r,source) %>% summarise(
  median = quantile(R_t,0.5),
  lower = quantile(R_t,0.025),
...
plot2data = compare_R %>% unnest(R) %>% glimpse()
```

```{r}
ggplot(R_t_summ )+
  geom_point(aes(y=median, x = factor(r), colour = source), position=position_dodge(width=0.4))+
  geom_errorbar(aes(ymin=lower, ymax=upper, x = factor(r), colour = source), width=0.2,position=position_dodge(width=0.4))+
  geom_point(data = plot2data, mapping=aes(y=`Median(R)`, x = factor(r), colour="epiestim (discr_si)"))+
  geom_errorbar(data = plot2data, mapping=aes(ymin=`Quantile.0.025(R)`, ymax=`Quantile.0.975(R)`, x = factor(r), colour="epiestim (discr_si)"), width=0.1)+
  xlab("Growth rate (days⁻¹)")+ylab("Reproduction number")
...
shape to the discrete distribution. In the Wallinga-Lispitch framework for
