...
simulations work, aggregate counts or case line lists simulations.
...
plot_changes = function(sim, name, max_y = NA, mapping = ggplot2::aes(), case=FALSE, fn = attr(sim,"fn"), changes = NULL, ...) {
  ...
  ggplot(mapping=mapping)+
    ...
    {if (is.null(fn)) NULL else geom_function(fun = ~ fn(t=as.time_period(.x, sim$time)), xlim=as.Date(range(sim$time)), n = length(sim$time))}+
...
ggplot()+geom_function(fun = fn, xlim = c(0,100))+ylab("growth rate")

```

`cfg_linear_fn` produces a very similar effect with less abrupt changes in $R_t$
and changes occuring such that the value at the knot is correct.
...
ggplot()+
  geom_function(fun = fn, xlim = c(0,100))+ylab("reproduction number")+
...
.ts_evaluate(~ rnorm(.x,5,1), demo)
```

`ggoutbreak` includes some RNGs that are parametrised by mean and some form of
dispersion parameter.

```{r}

# A random Bernoulli parameterised by probability.
...
# where the mean is a time varying value.
...
plot(.ts_evaluate(hospitalisation_prob_fn, demo),ylab = "sampled probability")
...
p1=gamma_ip %>% plot_ip() + facet_wrap(~"gamma")
p2=emp_ip %>% plot_ip() + facet_wrap(~"empirical")
...
emp_samples = emp_sample_fn(2000)
hist(emp_samples,breaks = c(0,seq(0.5,12)))
```
...
# We wrap this in a a function to calculate a IP for each 
...
tibble(ip = delay_t, t=times) %>% unnest(ip) %>% glimpse() %>%
  ggplot()+geom_rect(aes(xmin=t,xmax=t+1,ymin=a0,ymax=a1,fill = probability))+
  xlab("time")+ylab("delay")
...
tibble(delay = delay, t=times) %>%
  ggplot()+geom_boxplot(aes(x=as.factor(t),y=delay),outliers = FALSE)+
  xlab("time")+ylab("delay")

```

These approaches can be applied to other delay distributions such as time to 
hospitalisation, and can be made to vary on other variables in the simulation 
such as variant class or potentially patient age (not yet implemented).

## Periodic functions

A periodic function most obviously could be used to generate seasonal forcing:

```{r}

# A simple to interpret and easy to make integrate to 0. 
growth_rate_fn = \(t) dplyr::case_when(
  t %% 365 < 40 ~ 0.1,
  t %% 365 < 80 ~ -0.1,
  TRUE ~ 0)
ggplot()+geom_function(fun = growth_rate_fn, xlim = c(0,365*2))+ylab("growth rate")
...
# for line list functions a RNG with weekly
delay_rng = cfg_weekly_gamma_rng(mean=gamma_means)

tibble::tibble(t=rep(1:7,100)) %>%
  mutate(sample = .ts_evaluate(delay_rng, .)) %>%
  group_by(t) %>%
  summarise(
    mean = mean(sample),
    sd = sd(sample)
...
# Aggregate count simulation cookbook
...
withr::with_options(list('day_zero'=Sys.Date()),{
  ...
  p1 = plot_changes(sim, "paramterised growth rate", date_breaks = "2 weeks") 
...
sim2 = dplyr::bind_rows(lapply(c(0,0.25,0.5,1), function(k) 
    ...
    mutate(ascertainment_noise = k) 
  )) %>%
  group_by(ascertainment_noise)

ggoutbreak::plot_counts(sim2, events = attr(sim,"events"), date_breaks = "2 weeks")+
  facet_wrap(~sprintf("randomness: %1.2f",ascertainment_noise),ncol=2)
...
p1=plot_changes(sim3,name = "delays (days)", fn = ip_mean_fn)+ylim(0,NA)
p2=plot_changes(sim3,name = "detection", fn = p_detection_fn)+ylim(0,1)
p3=plot_counts(sim3,events = attr(sim3,"events"),mapping=aes(colour=statistic))+geom_line()
...
p1=plot_changes(seasonal_sim, name = "Rt", fn = seasonal_rt_fn, max_y = 3)
p2=plot_counts(seasonal_sim, events = attr(seasonal_sim,"events"),mapping=aes(colour=statistic),size=0.25)+geom_line()+facet_wrap(~ statistic,ncol=1,scales = "free_y")
...
well known biases, e.g. right truncation. This simulation is directly
parametrised with a theoretical $R_t$ value; summary of the network edges gives
us a realised case reproduction number which is specific for a simulation, and
the force of infection gives us a instantaneous reproduction number estimate for
the simulation. From the parametrised infectivity profile we can again use the
methods of Wallinga and Lipsitch to solve for a theoretical growth rate.
...
plot_cases(linelist, events = attr(linelist,"events"),individual=TRUE)+coord_cartesian()
...
p2=plot_counts(summary,events = attr(summary,"events"), mapping=aes(colour=statistic),size=0.5)+geom_line()
...
variant_Rt_fn = \(t,variant) case_when(
...
comparison = bind_rows(
  scenario1 %>% sim_summarise_linelist(variant) %>% mutate(scenario = "1 - Shorter GT"),
  scenario2 %>% sim_summarise_linelist(variant) %>% mutate(scenario = "2 - Rt advantage")
)

p1=plot_changes(comparison,"Rt", max_y = 2.5,mapping = aes(colour=variant))+facet_wrap(~scenario)
p2=plot_counts(comparison %>% group_by(scenario,variant), mapping = aes(colour=variant))+facet_wrap(~scenario)
...
summary1 =  scenario %>% sim_summarise_linelist(variant) %>% filter(time >= 0) 
summary2 =  scenario %>% sim_summarise_linelist() %>% filter(time >= 0)

p1=plot_counts(summary1, mapping=aes(colour=variant))
p2=plot_counts(summary2)

p3=ggplot2::ggplot(summary1, ggplot2::aes(x=as.Date(time), colour=variant))+
    ggplot2::geom_point(aes(y=rt.inst))+
    ggplot2::geom_line(aes(y=rt.weighted))+
...
comparison = bind_rows(lapply(kappa, function(k) {
  ...
  sim_branching_process(
    ...
    mutate(scenario = sprintf("Dispersion: %1.2f",k))
  
}))

plot_counts(comparison %>% group_by(class=scenario))+scale_y_log1p()
```
...
strat_imports_df = tibble::tribble(
  ...
  0:4, "child", 5,
  0:4, "adolescent", 5
) %>% mutate(age_cat = factor(age_cat,levels = c("child","adolescent","adult","elderly")))
...
plot_cases(strat_bpm,mapping = aes(fill=age_cat),individual=TRUE)+coord_cartesian()
...
#'   fn_symptom_delay = ~ rgamma(.x, shape = 3),
#'   fn_p_tested = \(symptomatic,...) ifelse(symptomatic, 0.8, 0.1),
#'   fn_sample_delay = ~ rgamma(.x, shape = 2),
#'   fn_result_delay = \(sample_time,...) dplyr::case_when(
#'       floor(sample_time) %% 7 %in% c(0,1,2,3,4) ~ rgamma(sample_time, shape=2), # Sun, Mon to Thurs
#'       floor(sample_time) %% 7 == 5 ~ rgamma(sample_time, shape=4), # Fri
#'       floor(sample_time) %% 7 == 6 ~ rgamma(sample_time, shape=3), # Sat
#'   )
#' )
#' 
#' 
...
#'       !symptomatic ~ runif(symptomatic, max = 14),
...
#'
#' ggplot(tmp4 %>% filter(!is.na(count)), aes(x=time, colour=statistic))+
