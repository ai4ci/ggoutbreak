...
`ggoutbreak` has a set of infrastructure to allow weekly case counts to be 
used to estimate the reproduction number.
...
p2 = plot_growth_rate(incidence,events = events)+coord_cartesian(ylim=c(-0.05*7,+0.05*7))+
  # The simulation data includes the parametrised growth rate:
  geom_line(data = sim, mapping = aes(x=as.Date(time), y=growth), colour="red")
...
p2 = plot_growth_rate(incidence2,events = events)+coord_cartesian(ylim=c(-0.05,+0.05))
...
it we use a different method (Wallinga-Lipsitch) to convert the growth rate
