...
max_x = ceiling(stats::quantile(qgamma(0.95, gammas$shape, gammas$rate),0.75))
...
comparison = dplyr::bind_rows(
    ...
    gammas %>% dplyr::mutate(source = "mcmc")
  ) %>% mutate(source = factor(source,labels = c("mcmc","epiestim")))
...
quantiles = gammas %>% 
  dplyr::select(mean, sd) %>% 
  dplyr::reframe(across(tidyselect::everything(), ~ stats::quantile(p=c(0.025,0.5,0.975),.x))) %>% dplyr::mutate(p=c(0.025,0.5,0.975))
quantiles %>% glimpse()
...
comparison2 = dplyr::bind_rows(
    simulated2 %>% dplyr::mutate(source="resampled"),
    gammas %>% dplyr::mutate(source = "mcmc")
  ) %>% mutate(source = factor(source,labels = c("mcmc","resampled")))
...
tmp = original_disc %>% dplyr::mutate(source=factor(source, levels = c("mcmc","epiestim","resampled")))
...
original data derived (`mcmc`) or log-normally resampled with correlation
(`resampled`). The result of this can be examined by looking at `EpiEstim`
...
profiles (`mcmc`), `EpiEstim` resampled infectivity profiles (`epiestim`), and
correlated log-normal resampled infectivity profiles (`resampled`):
...
ggplot2::ggplot(compare_R %>% tidyr::unnest(R))+
  ggplot2::geom_point(aes(x=as.factor(r), colour=source, y=`Median(R)`), position=ggplot2::position_dodge(width=0.8))+
  ggplot2::geom_errorbar(aes(x=as.factor(r), colour=source, ymin=`Quantile.0.025(R)`, ymax=`Quantile.0.975(R)`), position=ggplot2::position_dodge(width=0.8), width=0.2)+
...
resampled tend to produce a slightly higher reproduction number estimate but
this is only noticeable at high values of the reproduction number. `resampled`
correlated log-normals do better but the difference is small. To produce the 
correlated log-normals we are assuming knowledge of the correlation between
mean and SD, which is not usually made available in published data.
...
distribution mean and SD parameters which is not what `EpiEstim` requires.
