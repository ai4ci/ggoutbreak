...
download.file("https://pmc.ncbi.nlm.nih.gov/articles/instance/9796165/bin/jiac317_supplementary_data.zip", tmp)

dir = tempdir()
unzip(tmp,"Supplementary_Data_sensitivity_summary.xlsx",exdir = dir)

modelled = readxl::read_excel(fs::path(dir,"Supplementary_Data_sensitivity_summary.xlsx"),sheet="All tests")
...
tmp = tmp %>% mutate(
  ...
  mutate(
    qfun = purrr::map(data2, ~ splinefun(x=.x$y, y=.x$x, method="monoH.FC"))
  ) %>%
  mutate(
    samples = purrr::map(qfun, ~ tibble::tibble(
...
download.file("https://static-content.springer.com/esm/art%3A10.1038%2Fs41467-020-20568-4/MediaObjects/41467_2020_20568_MOESM4_ESM.xlsx",tmp2)
raw = readxl::read_excel(tmp2,"Sheet1")

raw = raw %>%
  mutate(tau = `duration of symptoms in days`, positive = `virus culture result`=="POS")

model = glm(positive ~ splines::bs(tau), data = raw,family = "binomial")
tmp = predict(model,newdata = tibble(tau = 0:50), se.fit = TRUE)
...
covid_viral_shedding = df %>%
  ...
  mutate(
    samples = purrr::map2(mu, sigma, ~ tibble(
