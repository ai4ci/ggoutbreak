...
#' #'
#' #' @examples
#' #' tmp = ggoutbreak::england_covid_poisson %>%
...
#'     modelled,
#'     function(modelled, omega, window, ...) {
#'       df = modelled
...
#'         #   ),
#'         df %>%
#'           dplyr::mutate(imputed = FALSE) %>%
#'           dplyr::mutate(
#'             proportion.samples = purrr::map2(
#'               proportion.fit,
#'               proportion.se.fit,
#'               ~ .expit(stats::rnorm(boots, .x, .y))
#'             )
#'           )
#'       })
#'
...
#'           window_subset = tmp$proportion.samples[
...
#'           # The proportion
#'           if (any(na.omit(proportion) < 0 | na.omit(proportion) > 1)) {
#'             emit_warning <<- TRUE
#'           }
