...
i_timestamped = interfacer::iface(
  time = ggoutbreak::time_period ~
    "A set of events with a timestamp as a `time_period`"
)

# Can be grouped.
i_timeseries = interfacer::iface(
  time = ggoutbreak::time_period + group_unique ~
    "A (usually complete) set of singular observations per unit time as a `time_period`"
)

i_incidence_data = interfacer::iface(
  count = positive_integer ~
    "Positive case counts associated with the specified time frame",
  i_timeseries
)

i_censored_incidence_data = interfacer::iface(
  i_incidence_data,
  obs_time = ggoutbreak::time_period ~
    "The time of the observation of the time series, as a `time_period`",
  .groups = ~ obs_time + .
)
...
i_baseline_incidence_data = interfacer::iface(
  baseline_incidence = positive_double ~
    "Baseline raw incidence rate as count data"
)
...
i_incidence_per_capita_normalised = interfacer::iface(
  ...
  population_unit = double ~
    "The population unit on which the per capita incidence rate is calculated",
  time_unit = lubridate::as.period ~
    "The time period over which the per capita incidence rate is calculated"
)

i_proportion_data = interfacer::iface(
  denom = positive_integer ~
    "Total test counts associated with the specified time frame",
  i_incidence_data
)

i_multinomial_proportion_data = interfacer::iface(
  class = factor ~
    "A factor specifying the type of observation. This will be things like
variant, or serotype, for a multinomial model. Any missing data points are ignored.",
  i_proportion_data,
  .groups = ~class
)

# Internal validation of the one timeseries at a time including only
# necessary groupings functions.

i_censored_incidence_input = interfacer::iface(
  i_censored_incidence_data,
  .groups = ~obs_time
)
...
i_proportion_model = interfacer::iface(
  ...
  proportion.se.fit = positive_double ~
    "the standard error of proportion estimate on a logit scale",
  proportion.0.025 = proportion ~
    "lower confidence limit of proportion (true scale)",
  proportion.0.5 = proportion ~ "median estimate of proportion (true scale)",
  proportion.0.975 = proportion ~
...
i_prevalence_model = interfacer::iface(
  i_timeseries,
  prevalence.0.025 = proportion ~
    "lower confidence limit of prevalence (true scale)",
  prevalence.0.5 = proportion ~ "median estimate of prevalence (true scale)",
  prevalence.0.975 = proportion ~
    "upper confidence limit of prevalence (true scale)"
)
...
i_incidence_model = interfacer::iface(
  ...
  incidence.se.fit = positive_double ~
    "the standard error of the incidence rate estimate on a log scale",
  incidence.0.025 = positive_double ~
    "lower confidence limit of the incidence rate (true scale)",
  incidence.0.5 = positive_double ~
    "median estimate of the incidence rate (true scale)",
  incidence.0.975 = positive_double ~
    "upper confidence limit of the incidence rate (true scale)"
)

i_incidence_per_capita_model = interfacer::iface(
  i_timeseries,
  incidence.per_capita.fit = double ~
    "an estimate of the incidence per capita rate on a log scale",
  incidence.per_capita.se.fit = positive_double ~
    "the standard error of the incidence per capita rate estimate on a log scale",
  incidence.per_capita.0.025 = positive_double ~
    "lower confidence limit of the incidence per capita rate (true scale)",
  incidence.per_capita.0.5 = positive_double ~
    "median estimate of the incidence per capita rate (true scale)",
  incidence.per_capita.0.975 = positive_double ~
    "upper confidence limit of the incidence per capita rate (true scale)",
  population_unit = double ~
    "The population unit on which the per capita incidence rate is calculated",
  time_unit = lubridate::as.period ~
    "The time period over which the per capita incidence rate is calculated"
...
i_risk_ratio_model = interfacer::iface(
  ...
  risk_ratio.0.025 = positive_double ~
    "lower confidence limit of the excess risk ratio for a population group",
  risk_ratio.0.5 = positive_double ~
    "median estimate of the excess risk ratio for a population group",
  risk_ratio.0.975 = positive_double ~
    "upper confidence limit of the excess risk ratio for a population group"
...
i_rate_ratio_model = interfacer::iface(
  ...
  # risk_ratio.fit = double ~ "an estimate of the excess risk ratio for a population group on a logit scale",
  # risk_ratio.se.fit = positive_double ~ "the standard error of the excess risk ratio for a population group on a logit scale",
  rate_ratio.0.025 = positive_double ~
    "lower confidence limit of the rate ratio for a population group",
  rate_ratio.0.5 = positive_double ~
    "median estimate of the rate ratio for a population group",
  rate_ratio.0.975 = positive_double ~
    "upper confidence limit of the rate ratio for a population group"
  # baseline_proportion = proportion ~ "The population baseline risk from which the excess risk ratio is based"
)
...
i_risk_model = interfacer::iface(
  ...
  # risk_ratio.fit = double ~ "an estimate of the excess risk ratio for a population group on a logit scale",
  # risk_ratio.se.fit = positive_double ~ "the standard error of the excess risk ratio for a population group on a logit scale",
  risk.0.025 = positive_double ~
    "lower confidence limit of the excess risk ratio for a population group",
  risk.0.5 = positive_double ~
    "median estimate of the excess risk ratio for a population group",
  risk.0.975 = positive_double ~
    "upper confidence limit of the excess risk ratio for a population group"
  # baseline_proportion = proportion ~ "The population baseline risk from which the excess risk ratio is based"
)
...
i_sim_count_data = interfacer::iface(
  statistic = character ~
    "An identifier for the statistic, whether that be infections, admissions, deaths",
  i_incidence_data,
  .groups = ~ statistic + .
)
