...
infer_prevalence = function(
  ...
  out = interfacer::igroup_process(modelled, function(modelled, logomega, window,...) {
    ...
    for (i in seq_along(tmp$foi.logsamples)) {
      if (i>=window+start && i<=end) {
        ...
        if (any(stats::na.omit(proportion) < 0 | stats::na.omit(proportion) > 1)) emit_warning<<-TRUE
    ...
    tmp2 = tmp %>%
      ...
      ) %>% dplyr::select(-incidence.logsamples, -foi.logsamples, -prevalence.samples)
