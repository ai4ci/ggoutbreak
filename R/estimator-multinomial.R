...
#' }
#'
multinomial_nnet_model = function(
  ...
  modelled = interfacer::igroup_process(
    d,
    function(d, ..., window, frequency, predict) {
      # remove zero count time points as these crash the algorithm
      tmp2 = d %>%
        dplyr::group_by(time) %>%
        dplyr::filter(sum(count) > 0) %>%
        dplyr::ungroup()
      tmp2 = tmp2 %>%
        dplyr::select(time, class, count) %>%
        tidyr::pivot_wider(
          names_from = class,
          values_from = count,
          values_fill = 0
      ...
      response = tmp2 %>%
        dplyr::select(-dplyr::any_of(c("time"))) %>%
        as.matrix()
      ...
      model = nnet::multinom(
        prob ~ splines::ns(time, df = df),
        Hess = TRUE,
        data = data
      )

      if (!predict) {
        return(dplyr::tibble(proportion = list(model)))
      }
